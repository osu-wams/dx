...
import { List, ListItem, ListItemContentLink, ListItemDescription, ListItemHeader, ListItemText } from '../ui/List';
...
const PlannerItems = () => {
  ...
  return (
    <Card>
      <CardHeader title="Canvas" badge={<CardIcon icon={faFileEdit} count={plannerItems.length} />} />
      <CardContent>
        ...
        {plannerItems.length && user.isCanvasOptIn === true ? (
          <List>
            {plannerItems.map(({ plannable_id, plannable_type, html_url, plannable: { title, due_at } }) => (
      ...
      <CardFooter>
        {user.isCanvasOptIn === true && <ExternalLink href={Url.canvas.main}>View all in Canvas</ExternalLink>}
      </CardFooter>
    </Card>
  );
};
